...
- **Security fix: a knowledge-base-scoped token could read source passages from other knowledge bases of the same account.** A scoped token sees only the knowledge bases it was granted - that has held for the graph tools (`list_graphs`, `search_entities`, `get_entity`, `get_neighbors`, `shortest_path`) since July. The passage paths were missed: `search_chunks`, the blended `query` / Talk-to-Graph answer and the grounding snippets returned with `get_entity` and `get_dossier` were limited to the account owner and the clearance rank only. We found it in an end-to-end test of an integration: a fresh token with a single grant on an empty knowledge base received passages from a colleague's audit report. Other accounts were never reachable, and clearance ranks were always enforced - the missing boundary was the knowledge base inside one account. Every passage read now goes through the same job allow-list the graph tools use, the blended answer is grounded on and cites only passages inside the token's scope, and a guard test fails the build if a future retrieval path forgets the scope. We recommend updating promptly if you hand scoped tokens to colleagues, agents or integrations. Details and a two-minute live check: `docs/security/2026-09-17-chunk-scope.md` in the repository.
- **`compilationId` is now a hard filter for retrieval.** `search_chunks` and `query` used to treat it as a preference and quietly fall back to the whole account when the knowledge base had no hit, so an answer "from knowledge base X" could rest on knowledge base Y. They now return what X holds and nothing else - an out-of-scope or unknown id yields an empty result. A Knowledge Wiki resolves to the passages of its source knowledge bases, so wiki chats keep working. If you relied on the old fallback, call without `compilationId`.
- **`search_chunks` takes a `limit`.** Up to 50 passages per call (default 5, as before), so an integration that fuses several searches no longer has to work with five.
- **Rollout note.** The API enforces the scope on its own, so updating `gctrl-api` closes the hole even before `gctrl-kex` is updated; the new KEX image then filters at the source, which restores full recall for tokens whose grants cover a small part of a large account.

## v0.9.11 - The Knowledge Wiki stays visible on large instances

*17 September 2026 · [GCTRL Team / TortillaJackson](https://github.com/TortillaJackson)*

- **The Wiki page finds your wiki no matter how many knowledge bases you have.** The page loaded the 100 newest knowledge bases and picked the wikis out of that list in the browser. On an instance that had grown past 100 knowledge bases - easy to reach once an integration such as Anvil files a knowledge base and a code graph per project - an older Knowledge Wiki fell out of that window and the page showed "No wikis yet", although the wiki was intact and had been distilled the same day. The list endpoint now filters by type on the server (`GET /api/kg/compilations?type=WIKI`, also `RAW` and `CODE`; `total` respects the filter), and the Wiki page asks for exactly the wikis, plus exactly the raw knowledge bases for the source picker, which had the same blind spot. The wiki picker for auto-distill schedules on the Triggers page was affected in the same way and is fixed with it. Nothing was lost and nothing needs to be re-distilled - the wiki reappears with the update.
- **The version shown in your update notice keeps pace with the release again.** Finding an update never depended on us: each instance compares its local images with the registry. But the version number and changelog next to that notice come from `gctrl.tech/version.json`, and that file had stood still since 11 September because the website deploy logged in to the registry with a 90-day access token that expired unnoticed. The deploy now uses the short-lived token of the release job itself and logs out again when it is done, so there is no credential left that can quietly run out. A second release step that leaned on the same token only rewrote a version marker in our deploy repository that no installer reads; it is removed rather than re-credentialed.
...
- **A busy queue is no longer reported as a failure.** The job watchdog failed every upload that had waited more than ten minutes as "Queue stalled" - even while the single extraction worker was busy completing the queue in front of it. A folder import of 69 documents on one instance saw 43 of them, every image among them, flagged as failed at minute ten and then quietly finished by the worker afterwards. Pending jobs are now only failed when the worker itself has made no progress for ten minutes (or after six hours regardless); a long queue simply waits, visibly, as pending.
...
- **Hot memory no longer empties itself overnight.** The memory governance cycle archived any non-pinned dossier whose heat had decayed for one idle hour - about eight hours after it was built - and any passage that had grounded exactly one answer some three hours later. An office that stops at five found every dossier archived by nine: the next `get_dossier` answered "no dossier" and rebuilt the same profile through the LLM, and every passage that had ever been cited had quietly left lexical retrieval. On the two instances we checked that left 1 dossier live against 49 archived, and 0 against 443. Eviction is now a long-idle decision - seven idle days for a dossier, thirty for a passage - a dossier that was never read is never evicted, an archived dossier is revived on access instead of rebuilt, and a migration brings back everything the old policy took while leaving deduplicated duplicates (whose vectors are gone) archived.
- **Use strengthens, disuse forgets.** GCTRL's memory tiers now follow the Hebbian rule. Every read path reinforces what it returned - the chat answer, an agent's `search_chunks`, `get_entity`, `get_dossier` and a `memory_feedback` vote - weighted by rank and by how strong the signal is; until now only the chat path left a trace, so a memory used exclusively through agents could never learn what those agents actually use. Each use also lengthens the item's own half-life: fresh knowledge halves in a week, knowledge that keeps being used decays on a scale of months (capped at 180 days), and the fixed "lose five percent every ten minutes" decay is gone. A rejected fact (`memory_feedback` down) drops out of the hot set at once.
- **Passages that fire together wire together.** The passages returned in one retrieval are linked as co-activated, with a bounded Hebbian weight that grows on every joint hit and fades with a thirty-day half-life. At search time a strongly linked neighbour of the hits is pulled into the result, so a fact that always mattered together with the one you asked about comes along even when the query did not mention it.
...
- **The memory snapshot shows the new layer.** `memory_health` reports the number of co-activation pairs, how many are strong, and the average half-life of the knowledge in use, next to the heat and trust distributions it already had. Settings → Memory shows the same four numbers as tiles under "Association & consolidation", and the last-run breakdown lists decayed and pruned pairs.
...
*4 September 2026 · [GCTRL Team / TortillaJackson](https://github.com/TortillaJackson)*

- **MLX / oMLX is now a selectable runtime.** Settings → Infrastructure lists "MLX / oMLX (Apple Silicon, this host)" next to Ollama, llama.cpp and vLLM: point it at the server on your Mac (the default is `http://host.docker.internal:8020`), paste the server's API key, and every purpose that inherits the global runtime - chat, Talk to Graph, relation extraction, wiki distillation, dossiers and the user profile - runs on it. Batch workers authenticate against a keyed server through an internal credential lookup; the key never enters the job queue.
- **A busy model server no longer looks like a dead one.** Under memory pressure an inference server answers with "busy, retry later" style codes (409, 429, 503, 507). GCTRL used to count three of those as three failures and silently switch the whole platform back to bundled Ollama, where the configured model does not even exist, and left an alarm banner up for a day. Those responses are now retried with backoff on every path (API, extraction, fusion), never count toward the auto-revert, and the guardrail only ever reverts when bundled Ollama is proven able to take over. Re-applying a runtime clears stale banners.
...
- **Hot memory no longer empties itself overnight.** The memory governance cycle archived any non-pinned dossier whose heat had decayed for one idle hour - about eight hours after it was built - and any chunk that had grounded exactly one answer about three and a half hours later. An office that stops at five found every dossier archived by nine: the next `get_dossier` answered "no dossier", rebuilt the same profile through the LLM, and every passage that had ever been cited had silently left lexical retrieval. On one live instance that left 1 dossier live against 46 archived and 1,141 archived chunks. Eviction is now a long-idle decision - seven idle days for a dossier, thirty for a chunk - heat keeps ranking, an archived dossier is revived on access instead of rebuilt, and the migration brings back everything the old policy evicted while leaving deduplicated duplicates (whose vectors are gone) archived.
- **Folder counts say what the folder shows.** A folder card counted every knowledge base of the account - across clearance caps, token grants, KB scope and the Codebase-access switch - while opening the folder listed only the twenty newest graphs of the whole account, filtered in the browser. A card could read "7 graphs" over an empty folder, and a KB-scoped colleague token learned the size of graphs it may not see. Counts now use the same visibility rule as the list, the card separates "graphs here" from "in subfolders", and the folder view asks the server for exactly that folder (`GET /api/kg/compilations?folderId=…`, `root` for unfiled graphs). The list's default page grew from 20 to 100 (max 500) - the FUSE picker, the Access page and the published MCP server's `list_graphs` all read that page.
- **Agents see where a knowledge base lives.** `list_graphs` on the agent gateway returned only id, name and classification, so a model told to "pick your knowledge base" had to guess between "Team Wave" and "Team Wave (code)". Every entry now carries `type` (RAW, WIKI, CODE), `folderId` and `folderPath` (`Users/<name>/Code`, `Projects/<client>/…`, `Global/…`), live node/edge counts and the number of sources, and the skill explains the placement rule: personal learnings under `Users/<you>`, confirmed team facts in the project, code decisions in the repository's CODE graph.
- **Users are managed from Settings.** A new Users tab (administrators only) lists every account with role, clearance, verification, tier, token balance, number of graphs and API keys and when it registered, with a seven-day registration counter - and adds the switch that was missing from the product entirely: activate or deactivate an account (`PUT /api/admin/users/:id/active`). Deactivation was enforced on login, on every request and on API-key auth, but could only be triggered through SCIM. An administrator cannot deactivate their own account.
