...
In the GCTRL portal: **Settings → Access Control → New Token**. Pick a clearance ceiling, grant the knowledge bases the agent may use, and toggle **KB-scoped** mode. Leave **Codebase access** on for an agent that should work with code knowledge bases and the code tools. Switch it off and that one token loses the code tools, CODE knowledge-base visibility (lists, grants, an explicitly named id, graph reads, chunk reads and RAG answers), code writes and every code knowledge-base change; a full-owner token with the flag off is narrowed to job-scoped reads of its non-code knowledge bases. The token is sent as:
