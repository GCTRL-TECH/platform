...
(distiller / dossier / user_profile are synchronous), but both are included for
consistency so the two modules (kex/fuse) can evolve in lockstep — they are
separate deployables and cannot share code (same helper names on both sides).
...
Thinking switch (`_think_flag()`, env FUSE_THINK, default off):
  - Ollama: `think: <bool>` per-request field (always sent).
  - /v1:    `chat_template_kwargs: {"enable_thinking": <bool>}` (always sent;
            qwen3/oMLX/vLLM honour it, others ignore the unknown key).

Transient-error contract (shared with api-rs and the KEX client):
  ...
  - After exhaustion the LAST error is raised unchanged (`requests.HTTPError`
    ...
    callers' fallback logic (distiller `_fallback_body`, dossier fact list) is
    untouched.
...
Per-base concurrency gate (openai kinds only):
  - One process-local `threading.Semaphore` per `_v1_root(base)`, sized by the
    `max_concurrency` kwarg (payload / request `generation_max_concurrency`) with
    env fallback GENERATION_MAX_CONCURRENCY (default 4). Stops ONE worker fanning
    N pages at once into a memory-tight local server (oMLX/vLLM/llama.cpp).
    Ollama and cloud providers are not gated. The async variant shares the same
    semaphore (acquired off-loop via asyncio.to_thread).
...
def _think_flag() -> bool:
    ...
    Ollama `think: false` / OpenAI-compatible `enable_thinking: false`, so a
    reasoning model (gemma3/gemma4, gpt-oss, qwen3, …) skips a chain-of-thought the
    distiller/summariser discards anyway (big speed-up; models without a thinking
    capability ignore the field, and older Ollama ignores the unknown key). Set
    FUSE_THINK=true to keep reasoning on FUSE's LLM calls."""
...
def complete(
    ...
    with telemetry.span(
        ...
        out = _complete_impl(
            prompt, model, base, kind, api_key=api_key, options=options,
...
async def acomplete(
    ...
    credential fetch); included for consistency with the KEX module (both
    deployables expose identical interfaces). Raises `httpx.HTTPStatusError` /
