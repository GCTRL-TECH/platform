...
export function GraphWorkspace() {
  ...
  useEffect(() => {
    ...
    // Score every node against the candidate mentions and pick the best. Shared
    // with the embed graph page — the exact scoring lives in lib/graph-focus.ts.
    const bestNode = matchFocusNode(nodes, candidates)
    if (bestNode) {
      focusApplied.current = true
      const name = (bestNode.label || (bestNode.properties?.name as string | undefined) || bestNode.id || '').trim()
      setSelectedName(name || bestNode.id)
      setContextOpen(true)
